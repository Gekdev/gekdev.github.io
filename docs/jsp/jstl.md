...
* 재사용 :  한번 작성한 커스텀 태그는 어떤 JSP 컨테이너에서도 사용이 가능

* 쉽고 단순한 JSP 코드 작성 : 자바 코드에 익숙하지 않은 개발자들도 커스텀 태그를 사용하면 쉽게 JSP 페이지를 작성할 수 있게 됨. 예를들어 JSTL과 EL을 사용하는 것이 스크립트 코드만 사용하기보다 쉬움

* 코드 가독성 향상 : 커스텀 태그는 뚜렷한 의도를 갖고 커스텀 태그를 사용하면 스크립트 코드를 줄일 수 있기 때문에 JSP 코드의 가독성을 높일 수 있음
