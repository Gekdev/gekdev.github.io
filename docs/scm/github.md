...
* git remote
    ...
    **원격 저장소를 관리할 수 있는 명령어, 주소를 등록한다.**
    ...
    !오류 {: .label .label-red .mt-3 } fatal: remote origin already exists.
...
* git pull
    : 다른 사람이 PR을 통해서 코드를 업데이트했거나, 아니면 Github를 통해서 commit했을 때 그 내용을 클라이언트로 내려받는 명령어 <br>
    
    충돌이(conflict) 일어나는 경우에는 충돌이 일어난 위치를 찾아서 해결(resolve)하고 다시 git pull로 가져오면 됨
